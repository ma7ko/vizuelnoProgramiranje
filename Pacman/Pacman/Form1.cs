        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < foodWorld.Length; i++)
            {
                for (int j = 0; j < foodWorld[i].Length; j++)
                {
                    ...
                    if(!canMove[i][j])
                    {
                        g.DrawImageUnscaled(Trap, j * Pacman.Radius * 2 + (Pacman.Radius * 2 - foodImage.Height) / 2, i * Pacman.Radius * 2 + (Pacman.Radius * 2 - foodImage.Width) / 2);
                    }
                }
            }
            pacman.Draw(g);
